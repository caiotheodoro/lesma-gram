...
export class UserActivityRepository implements UserActivityInterface {
  ...
  async createUserActivity({ userId,activityId,deliver,grade }: CreateUserActivityDTO): Promise<void> {
  ...
  async getUserActivityById(id:string): Promise<UserActivityEntity> {
    const response = await this.#pool.query('SELECT * FROM "user_activity" WHERE id = $1', [
      id,
    ]);
  ...
  async updateUserActivity({ id, userId, activityId, deliver,grade }: UpdateUserActivityDTO) {
    await this.#pool.query(
